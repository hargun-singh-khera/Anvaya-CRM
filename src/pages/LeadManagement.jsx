...
const LeadManagement = () => {
  ...
  const { data, loading: leadsLoading, error: leadsError } = useFetch(`https://neo-g-backend-9d5c.vercel.app/api/lead/${leadId}`)
  ...
  const handleSubmit = async (e) => {
    ...
    if (!commentText || !author) {
      return toast.error("Please select author and add a comment")
    }
    try {
      setLoading(true)
        ...
        <div className="col-md-8 mx-auto">
          <form onSubmit={handleUpdateLeads}>
            <table className="table">
              ...
              <tbody>
                {leadsLoading && Array.from({ length: 6 }).map((_, index) => <RenderTablePlaceholder key={index} />)}
                {!leadsLoading && !leads && <td colSpan={2} className="text-center"><p className="text-center py-4">No leads found.</p></td>}
            ...
            <button disabled={leadsLoading} className="btn btn-primary">{isEdit ? "Update Lead" : "Edit Lead Details"}</button>
          </form>
          <hr />
          <div>
            <h4 className="text-center my-4 mb-4">Comments Section</h4>
            {commentLoading && Array.from({ length: 5 }).map((_, index) => <CommentsPlaceholder key={index} />)}
            ...
            {!commentLoading && !commentError && commentsData?.length > 0 && commentsData?.map(comment => (
              <div key={comment._id} className="card p-2 rounded-3 mb-3">
